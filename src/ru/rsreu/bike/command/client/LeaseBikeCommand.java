...
public class LeaseBikeCommand implements ActivityCommand {
	...
	public Activity execute(HttpServletRequest request) {
		...
		tripDAO.createTrip(2, clientId, request.getParameter("id"));
		BikeDAO bikeDAO = (BikeDAO) request.getServletContext().getAttribute("BikeDAO");
		bikeDAO.book((String) request.getParameter("id"), "Y");

		return new Activity(
				CommandEnum.SELECT_BIKE.toString() + "&id=" + (String) request.getSession().getAttribute("addressId"),
